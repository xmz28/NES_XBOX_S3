...
/* Stop emulation at its own frame boundary.  Unlike main_quit(), this does
** not destroy the active machine while nes_emulate() is still on the stack. */
void main_requestquit(void)
{
   console.quit = true;
   if (system_nes == console.type)
      nes_poweroff();
}
