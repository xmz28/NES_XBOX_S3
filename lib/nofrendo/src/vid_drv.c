...
//      bmp_destroy(&back_buffer);
   ...
   /* The NES PPU sprite renderer may touch up to 8 pixels beyond either edge
   ** of a scanline.  Reboot-only builds hid the poisoned heap tail because
   ** this buffer was never freed; clean game exit requires the guard area. */
