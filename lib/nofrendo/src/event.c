...
void event_set_joypad1(unsigned char buttons)
{
   /* Replace the complete state every frame so a dropped edge cannot leave a
   ** direction or action button latched inside the emulator. */
   kb_input.data = buttons;
}
