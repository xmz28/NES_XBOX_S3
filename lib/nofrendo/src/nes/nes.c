...
{
   /* Audio belongs to every emulated frame, not only frames sent to the LCD.
   ** The display deliberately skips frames on slower targets. */
   gui_frame(draw);
   osd_pushaudio();

   /* USB state must reach the emulated joypad every emulated frame.  Tying
   ** this to vid_flush() made input latency follow the 12-20 FPS LCD path. */
   osd_getinput();
